...
from hrflow_connectors.v2.connectors.talentsoft.utils.const import (
    CIVILITY,
    CONTRACT_TYPE_REFERENTIAL,
    EDUCATIONS_REFERENTIEL,
    EXPERIENCES_REFERENTIEL,
)
from hrflow_connectors.v2.connectors.talentsoft.warehouse import TalentSoftWarehouse
...
from hrflow_connectors.v2.core.run import CallbackT, Event


def retrieve_tag_value(tags: t.List[dict], tag_name: str) -> t.Optional[str]:
    for tag in tags:
...
def format_ts_experiences(experiences: t.List[dict], tags: t.List[dict]) -> dict:
    ...
    if experience_level:
        experiences_ts = {
            "experienceLevel": EXPERIENCES_REFERENTIEL.get(experience_level),
...
def format_ts_vacancy_archive(ts_vacancy: t.Dict) -> t.Dict:
    return dict(
        reference=ts_vacancy["reference"],
    )
...
def format_ts_candidate_archive(ts_candidate: t.Dict) -> t.Dict:
    return dict(
        reference=ts_candidate["id"],
    )


def ts_callback(
    origin_parameters: AuthParameters,
    target_parameters: AuthParameters,
...
DESCRIPTION = (
    "Talentsoft (by Cegid) offers a full-suite HCM solution so you can keep all your HR"
    " processes in one platform. Make HR processes easy & simple for your teams. Help"
    " HR & IT leaders transform organizations. Accelerate HCM productivity for better"
    " performance."
)
TalentSoft = Connector(
    ...
    warehouse=TalentSoftWarehouse,
    flows=(
        Flow(
            Mode.create,
            Entity.profile,
            Direction.inbound,
            override_name="applicant_new",
            format=format_ts_candidate,
            event_parser=applicant_new_parser,
            callback=t.cast(
                CallbackT, ts_callback
            ),  # Explicit CallbackT requires the more general Parameters
        ),
        Flow(
            Mode.update,
            Entity.profile,
            Direction.inbound,
            override_name="applicant_update",
            format=format_ts_candidate,
        ...
        Flow(
            ...
            Entity.profile,
            Direction.inbound,
            format=format_ts_candidate_archive,
        ),
        Flow(
            Mode.create,
            Entity.job,
        ...
        Flow(
            ...
            Entity.job,
            Direction.inbound,
            format=format_ts_vacancy_archive,
        ),
        Flow(
            Mode.create,
            Entity.profile,
            Direction.outbound,
            format=format_info_ts_applicant,
        ),
        Flow(
            Mode.update,
            Entity.profile,
