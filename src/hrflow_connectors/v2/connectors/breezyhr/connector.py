...
from hrflow_connectors.v2.connectors.breezyhr.utils.datetime_converter import (
    from_str_to_datetime,
)
from hrflow_connectors.v2.connectors.breezyhr.utils.remove_html_tags import (
    remove_html_tags,
)
...
def format_jobs(breezy_job: t.Dict) -> t.Dict:
    """
                ...
                try:
                    ...
                    if isinstance(link, str):
                        if not re.match(r"http(s)?:\/\/.*", link):
                            ...
                            link = "https://" + link
                        breezy_profile["social_profiles"].update({type: link})
                except Exception:
                    continue
        ...
        if isinstance(attachments, list):
            for attachment in attachments:
                ...
                if isinstance(public_url, str):
                    breezy_profile["social_profiles"].update({file_name: public_url})
...
def format_candidate(breezy_profile: t.Dict) -> t.Dict:
    ...
    hrflow_profile = dict(info=info, experiences=[], educations=[], skills=[])
    ...
    for education in breezy_profile.get("education", []):
        ...
        educations.append(format_education)

    hrflow_profile["educations"] = educations
    ...
    hrflow_profile["experiences"] = experiences
...
BreezyHr = Connector(
    ...
    flows=(
        Flow(Mode.create, Entity.job, Direction.inbound, format=format_jobs),
        Flow(Mode.update, Entity.job, Direction.inbound, format=format_jobs),
        Flow(
        ...
        Flow(Mode.update, Entity.profile, Direction.outbound, format=format_profile),
        Flow(
            Mode.archive,
            Entity.profile,
            Direction.inbound,
        ...
        Flow(
            ...
            Entity.profile,
            Direction.outbound,
            format=format_archive_in_breezyhr,
        ),
    ),
)
