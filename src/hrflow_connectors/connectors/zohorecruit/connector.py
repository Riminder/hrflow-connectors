...
from hrflow_connectors.connectors.zohorecruit.warehouse import (
    ZohoRecruitCandidatesWarehouse,
    ZohoRecruitJobWarehouse,
)
...
def get_job_opening_tags(job_opening: JobOpening) -> list:
    tags_list = [
        "AccountManager",
        "AssignedRecruiter",
        "ClientName",
...
def format_job_opening_to_hrflow(job_opening: JobOpening) -> dict:
    hrflow_job = dict(
...
def get_candidate_tags(candidate: Candidate) -> list:
    tags_list = [
        "Current_Job_Title",
        "Current_Salary",
        "Expected_Salary",
...
def format_candidate_to_hrflow(candidate: Candidate) -> dict:
    hrflow_profile = dict(
...
def get_url(urls: list, url_type: str) -> str:
    for url in urls:
...
def format_hrflow_profile_to_zoho(profile: dict) -> Candidate:
    zoho_candidate = dict(
...
ZohoRecruit = Connector(
    ...
    actions=[
        ConnectorAction(
            ...
            description=(
                "Retrieves profiles from Zoho Recruit and writes them to an Hrflow.ai"
                " source"
            ),
            parameters=BaseActionParameters.with_defaults(
                "ReadProfileActionParameters", format=format_candidate_to_hrflow
            ),
            origin=ZohoRecruitCandidatesWarehouse,
        ...
        ConnectorAction(
            ...
            target=ZohoRecruitCandidatesWarehouse,
        ...
        ConnectorAction(
            ...
            parameters=BaseActionParameters.with_defaults(
                "ReadJobActionParameters", format=format_job_opening_to_hrflow
            ),
            origin=ZohoRecruitJobWarehouse,
