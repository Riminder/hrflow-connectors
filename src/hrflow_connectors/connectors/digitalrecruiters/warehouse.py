...
from hrflow_connectors.connectors.digitalrecruiters.schema import DigitalRecruitersJob, DigitalRecruitersReadProfile,DigitalRecruitersWriteProfile
...
DIGITAL_RECRUITERS_WRITE_PROFILES_ENDPOINT = "{url_environnement}/api/candidate/apply/{token}"
...
def read_jobs(
    ...
    url = DIGITAL_RECRUITERS_JOBS_ENDPOINT.format(url_environnement=DR_JOB_URL, token=parameters.digital_recruiters_token)
    ...
    if token:
        ...
        while next_page_link:
            try:
                ...
                adapter.info("Token expired. Refreshing...")
...
def write(
    ...
    url = DIGITAL_RECRUITERS_WRITE_PROFILES_ENDPOINT.format(url_environnement=DR_PROFILES_URL, token=parameters.digital_recruiters_token)	
...
DigitalRecruitersReadProfilesWarehouse = Warehouse(
    ...
    read=WarehouseReadAction(
        ...
        endpoints=[ActionEndpoints(name="Read Profiles", description="Read profiles from Digital Recruiters", url=DIGITAL_RECRUITERS_READ_PROFILES_ENDPOINT)],
    ),
)
DigitalRecruitersWriteProfileWarehouse = Warehouse(
    ...
    write=WarehouseWriteAction(
        parameters=WriteProfilesParameters,
        function=write,
        endpoints=[ActionEndpoints(name="Write Profile", description="Write profile to Digital Recruiters", url=DIGITAL_RECRUITERS_WRITE_PROFILES_ENDPOINT)],
    ),
)
