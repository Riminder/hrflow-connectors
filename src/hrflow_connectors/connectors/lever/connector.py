...
def extract_job_sections(contents: list) -> str:
    ...
    for item in lists:
        ...
        section = dict(name=item.get("text", None),title = item.get("text", None), description=content_text)
        if section:
...
def get_job_tags(lever_job: dict) -> list:
    ...
    tags.append({"name": "lever_confidential", "value": "False" if lever_job.get("confidentiality", "") == "non-confidential" else "True"})
    
    categories = lever_job.get("categories", {})
    tags.extend({"name": f"lever_{tag_name}", "value": tag_value} for tag_name, tag_value in categories.items())
    
    additional_tags = lever_job.get("tags", [])
    tags.extend({"name": f"lever_additional_tag_{i}", "value": tag} for i,tag in enumerate(additional_tags))
    
    distribution_channels = lever_job.get("distributionChannels", [])
    tags.extend({"name": f"lever_distributionChannel_{index}", "value": channel} for index , channel  in enumerate(distribution_channels))
    
    urls = lever_job.get("urls", {})
    tags.extend({"name": f"lever_url_{item}", "value": value} for item, value in urls.items())
...
def format_job(lever_job: dict) -> dict:
    ...
    created_at = datetime.fromtimestamp(int(lever_job_data["createdAt"]) / 1000).isoformat()
    updated_at = datetime.fromtimestamp(int(lever_job_data["updatedAt"]) / 1000).isoformat()
	
    job = dict(
        reference=lever_job_data["id"],
        name=lever_job_data["text"],
        location=dict(
				text=lever_job_data.get("categories", {}).get("location", None) if lever_job_data.get("categories", {}).get("location", None) else None,
				lat=None,
				lng=None
			),
...
def get_profile_experiences(lever_profile_data: t.Dict[str, any]) -> t.List:
    ...
    for experience_data in experiences_data:
        ...
        start_date = datetime(start_year, start_month, 1).isoformat() if start_year and start_month else None
        end_date = datetime(end_year, end_month, 1).isoformat() if end_year and end_month else None
...
def get_profile_attachments(lever_profile_data: t.Dict[str, any]) -> t.List[str]:
    ...
    if attachments_data:
        ...
        attachment_dict["created_at"] = datetime.fromtimestamp(int(attachments_data["uploadedAt"]) / 1000).isoformat()
...
def format_profile(opportunity_data: dict) -> dict:
    ...
    lever_profile = lever_opportunity.get("profile")[0] if lever_opportunity.get("profile") else {}
    created_at = datetime.fromtimestamp(int(lever_opportunity["createdAt"]) / 1000).isoformat()
    updated_at = datetime.fromtimestamp(int(lever_opportunity["updatedAt"]) / 1000).isoformat()
    ...
    profile = dict(
        ...
        info={
            ...
            "last_name": last_name,
            "email": lever_opportunity.get("emails")[0] if lever_opportunity.get("emails") else None,
            "phone": lever_opportunity.get("phones")[0].get("value") if lever_opportunity.get("phones") else None,
            "location": dict(text=lever_opportunity.get("location"), lat=None, lng=None) if lever_opportunity.get("location") else dict(text=None, lat=None, lng=None),
            "urls": format_profile_urls(lever_opportunity.get("links")),
        },
        text_language=None,
        text=lever_opportunity.get("headline") if lever_opportunity.get("headline") else None,
...
def format_opportunity(hrflow_profile : dict) -> dict:
    ...
    lever_opportunity["phones"] = [{"type": "mobile", "value": hrflow_profile["info"]["phone"]}]
...
DESCRIPTION = "Lever is a modern recruitment platform that helps companies streamline their hiring process."
Lever = Connector(
    name="Lever",
    description=DESCRIPTION,
    ...
    actions=[
        ConnectorAction(
            ...
            description="Retrieves all jobs via the Lever API and sends them to the Hrflow.ai Board.",
