...
class MonsterProfile(BaseModel):
    ...
    FileExt: str = Field(
        ...,
        Description=(
            "This contains the resume in its original format as it was made available"
            ...
            " from the serialization / deserialization process, XML format should be"
            " set to accept up to 8 MB/ JSON format should be set to accept up to"
            " 20 MB."
        ),
    )
    ...
    WorkAuthorization: int = Field(
        ...,
        Description=(
            "This is a value of whether the applicant is authorized to work "
