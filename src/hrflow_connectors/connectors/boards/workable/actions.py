...
class PullJobs(HTTPStream, BoardAction):
    subdomain: str = Field(..., description="subdomain of a company endpoint in `https://www.workable.com/api/accounts/{subdomain}` for example subdomain=`eurostar` for eurostar company")
    ...
    def pull(self)-> Iterator[Dict[str, Any]]:
        ...
        if response.status_code == 200:
            ...
            if total_found == 0:
                ...
                raise Exception(f"This company has no jobs available on their workable public endpoint")
            else:
                logger.info(f'Total jobs found for this request: {len(job_dict_list)}')
                return job_dict_list
        else:
            logger.error(f"Failed to get jobs from subdomain: {self.subdomain}. Check that the subdomain is a valid one")
    ...
    def format(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        city = data.get('city')
        job['location'] = dict(text=city, country=country, state=state, lat=None, lng=None)
        job['url'] = data.get('url')
        description = remove_html_tags(data.get('description'))
        job['sections'] = [dict(name='workable_description', title='workable_description', description=description)]
