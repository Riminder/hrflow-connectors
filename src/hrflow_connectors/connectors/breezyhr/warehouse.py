...
class BreezyhrReadParameters(ParametersModel):
    ...
    # Add more specification manifest.json
    class Config:
        @staticmethod
        def schema_extra(
            schema: t.Dict[str, t.Any], model: t.Type["BreezyhrReadParameters"]
        ) -> None:
            anyOf = []
            for key, value in schema.get("properties").items():
                if value.get("field_type") == FieldType.AnyOf:
                    anyOf.append(key)
            if anyOf:
                schema.update({"anyOf": anyOf})
