...
class FlatchrCreationProfile(BaseModel):
    ...
    token: str = Field(..., Description="API key. It is created in the Flatchr interface")
    resume: FlatchrResume = Field(..., Description="Type of format for the CV: document (if cv base64, default), "
                                                   "json (if cv, hr-xml format)")
    email: str = Field(..., Description="Email will be used as a reference for the profile. The enrichment request "
    ...
    legalNewsletterPartners: Optional[bool] = Field(..., Description="Opt-in newsletter")
    ...
    answers: Optional[FlatchrAnswers] = Field(..., Description="Answers to questions/tags")


class FlatchrAdressProfile(BaseModel):
    formattedAddress: str = Field(..., Description="Adress where the education were done")


class FlatchrCommunicationProfile(BaseModel):
    address: List[FlatchrAdressProfile] = Field(..., Description="List of adresses where the education were done")


class FlatchrInstitutionProfile(BaseModel):
    communication: FlatchrCommunicationProfile = Field(..., Description="List of adresses")
    name: str = Field(..., Description="Name of the institution in which the education was provided")
...
class FlatchrEducationProfile(BaseModel):
    institution: FlatchrInstitutionProfile = Field(..., Description="Institution object")
    educationLevelCodes: List[FlatchrEducationLevelCode] = Field(..., Description="List of all the degrees")
    educationDegrees: List[FlatchrEducationDegree] = Field(..., Description="List of all the degrees")
...
class FlatchrAddres1(BaseModel):
    ...
    geoLocation: FlatchrGeoLocation = Field(..., Description="GeoLocation object of the employment location")
...
class FlatchrOrganization(BaseModel):
    communication: FlatchrCommunication1 = Field(..., Description="Communication1 object")
...
class FlatchrPositionHistory(BaseModel):
    ...
    jobCategories: List[FlatchrJobCategory] = Field(..., Description="Category of the job")
...
class FlatchrEmploymentProfile(BaseModel):
    ...
    positionHistories: List[FlatchrPositionHistory] = Field(..., Description="List of the position histories object")
    start: str = Field(..., Description="Date of the start of the employment")
    current: bool = Field(..., Description="Is this employment the current employment")
...
class FlatchrValueProfile(BaseModel):
    education: List[FlatchrEducationProfile] = Field(..., Description="List of educations")
    employment: List[FlatchrEmploymentProfile] = Field(..., Description="List of employments")
    ...
    email: List[FlatchrEmailProfile] = Field(..., Description="Email adress of the candidat")
    position: List[str] = Field(..., Description="Position of the candidat")
    employment_positions: List[str] = Field(..., Description="Employment adress of the candidat")
...
class FlatchrEnrichmentProfile(BaseModel):
    ...
    reference: str = Field(..., Description="Reference of the profile which is the email adress")
