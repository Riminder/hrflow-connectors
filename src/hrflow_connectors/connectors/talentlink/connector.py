...
from hrflow_connectors.connectors.talentlink.warehouse import (
    TalentLinkApplicationWarehouse,
    TalentLinkJobWarehouse,
    TalentLinkProfileWarehouse,
    TalentLinkScoreWarehouse,
)
...
TalentLink = Connector(
    ...
    actions=[
        ...
        ConnectorAction(
            ...
            trigger_type=WorkflowType.pull,
            description=(
                "Retrieve all applications from TalentLink and send them to an"
                " Hrflow.ai Source."
            ),
            parameters=BaseActionParameters.with_defaults(
                "ReadApplicationsActionParameters", format=format_tl_application
            ),
            origin=TalentLinkApplicationWarehouse,
            target=HrFlowProfileWarehouse,
            action_type=ActionType.inbound,
        ),
        ConnectorAction(
            name=ActionName.push_score_list,
            trigger_type=WorkflowType.catch,
            description="Push a list of scores to TalentLink via the API",
            parameters=BaseActionParameters.with_defaults("WriteScoreActionParameters"),
            origin=HrFlowProfileWarehouse,
            target=TalentLinkScoreWarehouse,
            action_type=ActionType.outbound,
        ),
    ],
)
