        ...
        while True:
            try:
                ...
                for job in data:
                    ...
                    if (action == "create" and job.get("dateAdded") != job.get("dateLastModified")) or \
                        (action == "update" and job.get("dateAdded") == job.get("dateLastModified")): 
                        continue


                    if last_id and job[bullhorn_date_field] == date_value and job["id"] <= last_id:
...
def item_to_read_from_job_create(item: t.Dict) -> str:
    ...
    return json.dumps(
        dict(created_date=created_date, last_id=item["id"])
    )
