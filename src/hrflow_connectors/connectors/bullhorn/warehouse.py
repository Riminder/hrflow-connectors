...
class Array(ParametersModel):
    __root__: t.List[str] = Field(..., title="array",repr=False,field_type=FieldType.Other)
    ...
    def to_query_string(self) -> str:
        if isinstance(self.value, list):
            if self.operator == Operator.IN:
                ...
                raise ValueError(f"Operator {self.operator} is not valid for list values.")

        if self.operator == Operator.EQUAL:
            return f'{self.field}:{self.value}'  # For boolean and integer values like isOpen:true
...
class Query(ParametersModel):
    ...
    operator: t.Optional[LogicalOperator] = Field(
        None,
        description="Logical operator to combine conditions (optional if there's only one condition)",
        repr=False,
        field_type=FieldType.QueryParam,
    )
    ...
    def validate_operator(cls, v, values):
        if len(values['conditions']) > 1 and v is None:
            raise ValueError('Operator is required when there is more than one condition.')
        return v
    ...
    def to_query_string(self) -> str:
        ...
        condition_strings = [condition.to_query_string() for condition in self.conditions]
...
class ReadJobsParameters(ParametersModel):
    ...
    query: t.Union[String, Query] = Field(
        ...,
        description="A query string or a Query object with conditions combined using AND or OR",
        repr=False,
        field_type=FieldType.QueryParam,
    )
...
#     )
#     fields = (
#         "firstName,lastName,name,email,mobile,dateOfBirth,"
#         "gender,address,dateAvailable,status,employeeType,activePlacements,"
...
def transform_timestamp_read_from(timestamp: t.Optional[t.Union[float, int]]) -> t.Optional[str]:
...
def item_to_read_from_job(item: t.Dict) -> str:
    ...
    return json.dumps(
        dict(last_modified_date=last_modified_date, last_id=item["id"])
    )
