...
from hrflow_connectors.v1.core.connector import ActionName as ActionNameEnum
from hrflow_connectors.v1.core.connector import Connector, Event, Reason, Status
...
def warehouses(connector_subtype: str, connectors_directory: Path):
    if connectors_directory is CONNECTORS_DIRECTORY:  # pragma: no cover
        warehouse_module = import_module(
            "hrflow_connectors.v1.connectors.{}.warehouse".format(connector_subtype)
        )
    else:
