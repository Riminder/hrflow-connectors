...
class Parser:
    ...
    def items_fn(self):
        ...
        if self.current_token.kind != TokenType.ITEMS_FN.name:
            return res.failure(
                Error(
                    start=self.current_token.start,
                    end=self.current_token.end,
                    type=ErrorType.InvalidSyntax,
                    details="Expecting $items function but found {}".format(
                        self.current_token
                    ),
                )
            )
        ...
        if self.current_token.kind == TokenType.L_PAREN.name:
            return res.failure(
                Error(
                    ...
                    type=ErrorType.InvalidSyntax,
                    details=(
                        "Incorrect call of function $items. No arguments are expected"
                    ),
                )
            )

        return res.success(FunctionNode(fn=TokenType.ITEMS_FN, args=[]))
    ...
    def consumer(self):
        ...
        if token.kind in FUNCTION_TOKENS:
            ...
            if token.kind == TokenType.ITEMS_FN.name:
                items_fn = res.register(self.items_fn())
                if res.error:
                    return res
                return res.success(items_fn)
