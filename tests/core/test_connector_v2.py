...
from hrflow_connectors.core.connector_v2 import (
    ActionMode,
    ActionType,
...
from tests.core.src.hrflow_connectors.connectors.localusers.warehouse_v2 import (
    FAIL_AT,
    USERS_DB,
    BadUsersWarehouse,
...
from tests.core.src.hrflow_connectors.connectors.smartleads.warehouse_v2 import (
    LEADS_DB,
    BadLeadsWarehouse,
...
SmartLeadsF = lambda: Connector(
    ...
    actions=[
        ...
        ConnectorAction(
            trigger_type=WorkflowType.pull,
            name="create_incremental_users_in_hrflow",
...
# SmartLeadsOutboundF = lambda: Connector(
#     name="SmartLeads",
#     type=ConnectorType.Other,
#     subtype="smartleads",
#     description=DESCRIPTION,
...
#         ConnectorAction(
#             trigger_type=WorkflowType.pull,
#             name="archive_profiles_in_leads",
#             description="Action with support for incremental",
#             parameters=BaseActionParameters,
#             origin=UsersIncrementalWarehouse,
#             target=LeadsWarehouse,
...
def test_action_by_name():
    ...
    assert (
        SmartLeads.model.action_by_name("create_profiles_in_hrflow")
        is SmartLeads.model.actions[0]
    )
    assert (
        SmartLeads.model.action_by_name("create_jobs_in_hrflow")
...
def test_connector_failures():
    SmartLeads = SmartLeadsF()
    # source_key = "source_xxx1"
    campaign_id = "camp_xxx1"
    api_secret = "api_secret_xxx1"
    api_user = "api_user_xxx1@hrflow.ai"
    result = SmartLeads.create_profiles_in_hrflow(
    ...
    result = SmartLeads.create_profiles_in_hrflow(
        workflow_id=random_workflow_id(),
        connector_auth_parameters=dict(api_secret=1234),
        hrflow_auth_parameters=dict(),
        pull_parameters=dict(gender="M"),
        push_parameters=dict(),
    )
    ...
    result = SmartLeads.create_profiles_in_hrflow(
        workflow_id=random_workflow_id(),
        format=lambda user: 10 / 0,
        connector_auth_parameters=dict(),
        hrflow_auth_parameters=dict(api_user=api_user, api_secret=api_secret),
        pull_parameters=dict(),
        push_parameters=dict(campaign_id=campaign_id),
    )
    assert result.status == Status.fatal
    assert result.reason == Reason.format_failure
    result = SmartLeads.create_profiles_in_hrflow(
        workflow_id=random_workflow_id(),
        logics=[lambda user: 10 / 0],
        connector_auth_parameters=dict(),
        hrflow_auth_parameters=dict(api_user=api_user, api_secret=api_secret),
        pull_parameters=dict(),
        push_parameters=dict(campaign_id=campaign_id),
...
def test_origin_warehouse_failure():
    ...
    result = connector.create_profiles_in_hrflow(
        workflow_id=random_workflow_id(),
        connector_auth_parameters=dict(),
        hrflow_auth_parameters=dict(
            api_user="api_user_xxx1@hrflow.ai", api_secret="api_secret_xxx1"
        ),
        pull_parameters=dict(),
        push_parameters=dict(source_key="source_xxx1"),
    )
    assert result.status == Status.fatal
...
def test_origin_not_readable_failure():
    ...
    assert errors[0]["msg"] == "Origin warehouse is not readable for creation"
    ...
    errors = excinfo.value.errors()
    assert errors[0]["loc"] == ("origin",)
    assert errors[0]["msg"] == "Origin warehouse is not readable for update"
    ...
    errors = excinfo.value.errors()
    assert errors[0]["loc"] == ("origin",)
    assert errors[0]["msg"] == "Origin warehouse is not readable for archiving"
...
def test_target_warehouse_failure():
    ...
    result = connector.push_profile(
        ...
        pull_parameters=dict(),
        push_parameters=dict(campaign_id="camp_xxx1"),
    )
    assert result.status == Status.fatal
...
def test_target_not_writable_failure():
    ...
    assert errors[0]["msg"] == "Target warehouse is not writable for creation"
    ...
    errors = excinfo.value.errors()
    assert errors[0]["loc"] == ("target",)
    assert errors[0]["msg"] == "Target warehouse is not writable for update"
    ...
    errors = excinfo.value.errors()
    assert errors[0]["loc"] == ("target",)
    assert errors[0]["msg"] == "Target warehouse is not writable for archiving"
...
def test_connector_with_logics():
    ...
    result = SmartLeads.create_profiles_in_hrflow(
        ...
        pull_parameters=dict(),
        push_parameters=dict(campaign_id=campaign_id),
        logics=[lambda u: u, lambda u: None, lambda u: u],
    )
    ...
    result = SmartLeads.create_profiles_in_hrflow(
        ...
        pull_parameters=dict(),
        push_parameters=dict(campaign_id=campaign_id),
        logics=[lambda u: u, lambda u: u, lambda u: None],
    )
    ...
    result = SmartLeads.create_profiles_in_hrflow(
        ...
        pull_parameters=dict(),
        push_parameters=dict(campaign_id=campaign_id),
        logics=[lambda u: u, lambda u: u, lambda u: u],
    )
    ...
    result = SmartLeads.create_profiles_in_hrflow(
        ...
        pull_parameters=dict(),
        push_parameters=dict(campaign_id=campaign_id),
        logics=[lambda u: u, logic, lambda u: u],
    )
...
def test_action_with_failures():
    ...
    result = FailingSmartLeads.push_profile(
        ...
        pull_parameters=dict(),
        push_parameters=dict(campaign_id=campaign_id),
        format=failing_format,
        logics=[failing_logic],
    )
...
def test_action_with_callback_success():
    ...
    def callback(
        ...
        assert origin_auth_parameters.api_user == api_user
        assert origin_auth_parameters.api_secret == api_secret
        assert origin_action_parameters.gender.value == "male"
        assert target_auth_parameters.api_user == api_user
        assert target_auth_parameters.api_secret == api_secret
        assert target_action_parameters.campaign_id == campaign_id
    ...
    result = SmartLeads.push_profile(
        workflow_id=random_workflow_id(),
        connector_auth_parameters=dict(api_user=api_user, api_secret=api_secret),
        hrflow_auth_parameters=dict(api_user=api_user, api_secret=api_secret),
        pull_parameters=dict(gender="male"),
        push_parameters=dict(campaign_id=campaign_id),
    )
...
def test_connector_incremental_read_not_supported():
    ...
    result = SmartLeads.create_profiles_in_hrflow(
        ...
        connector_auth_parameters=dict(api_user=api_user, api_secret=api_secret),
        hrflow_auth_parameters=dict(api_user=api_user, api_secret=api_secret),
        pull_parameters=dict(read_mode=ReadMode.incremental),
        push_parameters=dict(campaign_id=campaign_id),
    )
...
def test_connector_incremental_backend_not_configured():
    ...
    with mock.patch.object(backend, "is_configured", new=False):
        with mock.patch.object(
            SmartLeads.model.actions[0].origin.create, "supports_incremental", new=True
        ):
            result = SmartLeads.create_profiles_in_hrflow(
                workflow_id=random_workflow_id(),
                connector_auth_parameters=dict(
...
#     workflow_id = random_workflow_id()
#     with mock.patch.object(
#         SmartLeads.model.actions[2].origin.create,
#         "item_to_read_from",
#         new=lambda item: 1 / 0,
#     ):
#         result = SmartLeads.create_incremental_users_in_hrflow(
...
#         connector_auth_parameters=dict(),
#         hrflow_auth_parameters=dict(),
#         pull_parameters=dict(read_mode=5),
#         push_parameters=dict(campaign_id=campaign_id),
#     )
#     assert result.status == Status.fatal
#     assert result.reason == Reason.bad_action_parameters
...
#         connector_auth_parameters=dict(),
#         hrflow_auth_parameters=dict(),
#         pull_parameters=dict(read_mode=ReadMode.incremental, gender="XFRE"),
#         push_parameters=dict(campaign_id=campaign_id),
#     )
...
#         hrflow_auth_parameters=dict(),
#         pull_parameters=dict(read_mode=ReadMode.incremental),
#         push_parameters=dict(campaign_id=[1, 2, 3]),
#     )
#     assert result.status == Status.fatal
#     assert result.reason == Reason.bad_target_parameters
#     assert backend.store.load(key=workflow_id, parse_as=RunResult).read_from == str(
...
def test_no_empty_parameters_overrride():
    with pytest.raises(ValidationError) as excinfo:
        ParametersOverride(name="push_job_list", format=None, event_parser=None)
...
def test_connector_based_on_simple():
    ...
    for action in SmartLeads.model.actions:
        assert hasattr(SmartLeadsCopy, action.name)


def test_connector_based_on_action_override():
    SmartLeads = SmartLeadsF()

    action_to_override = "create_profiles_in_hrflow"
    assert hasattr(SmartLeads, action_to_override)
...
def test_connector_based_on_new_action():
    ...
    new_action_name = "pull_resume_attachment_list"
    assert not hasattr(SmartLeads, new_action_name)
    ...
    assert new_action.origin == FailingUsersWarehouse
...
def test_connector_based_on_parameters_override():
    ...
    parameters_action_name = "create_incremental_users_in_hrflow"
    assert hasattr(SmartLeads, parameters_action_name)
...
def test_connector_based_on_bad_parameters_override():
    ...
    parameters_action_name = "push_job"
    assert not hasattr(SmartLeads, parameters_action_name)
