...
from tests.v1.core.src.hrflow_connectors.connectors.smartleads.warehouse import (
    LEADS_DB,
    LeadsWarehouse,
)
...
SmartLeads = Connector(
    ...
    actions=[
        ...
        ConnectorAction(
            ...
            action_type=ActionType.inbound,
            trigger_type=WorkflowType.catch,
            description="Test action with Event Parser",
...
def test_pull_workflow_code(with_smartleads, test_connectors_directory_v1):
    action_manifest = SmartLeads.manifest(test_connectors_directory_v1)["actions"][0]
...
def test_pull_workflow_code_with_format(with_smartleads, test_connectors_directory_v1):
    action_manifest = SmartLeads.manifest(test_connectors_directory_v1)["actions"][0]
    assert action_manifest["name"] == "push_profile_list"
    assert action_manifest["trigger_type"] == "schedule"
    assert "workflow_code_event_parser_placeholder" not in action_manifest
...
def test_pull_workflow_code_with_logics(with_smartleads, test_connectors_directory_v1):
    action_manifest = SmartLeads.manifest(test_connectors_directory_v1)["actions"][0]
    assert action_manifest["name"] == "push_profile_list"
    assert action_manifest["trigger_type"] == "schedule"
    assert "workflow_code_event_parser_placeholder" not in action_manifest
...
def test_catch_workflow_code(with_smartleads, test_connectors_directory_v1):
    action_manifest = SmartLeads.manifest(test_connectors_directory_v1)["actions"][1]
...
def test_catch_workflow_code_with_format(with_smartleads, test_connectors_directory_v1):
    action_manifest = SmartLeads.manifest(test_connectors_directory_v1)["actions"][1]
    assert action_manifest["name"] == "push_profile"
    assert action_manifest["trigger_type"] == "hook"
    assert "workflow_code_event_parser_placeholder" in action_manifest
...
def test_catch_workflow_code_with_logics(with_smartleads, test_connectors_directory_v1):
    action_manifest = SmartLeads.manifest(test_connectors_directory_v1)["actions"][1]
    assert action_manifest["name"] == "push_profile"
    assert action_manifest["trigger_type"] == "hook"
    assert "workflow_code_event_parser_placeholder" in action_manifest
...
def test_catch_workflow_code_with_event_parser(
    with_smartleads, test_connectors_directory_v1
):
    action_manifest = SmartLeads.manifest(test_connectors_directory_v1)["actions"][1]
    assert action_manifest["name"] == "push_profile"
    assert action_manifest["trigger_type"] == "hook"
...
def test_catch_workflow_code_with_default_event_parser(
    ...
    action_manifest = SmartLeads.manifest(test_connectors_directory_v1)["actions"][2]
